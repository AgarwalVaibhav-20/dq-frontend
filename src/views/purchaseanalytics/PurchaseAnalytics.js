...
//           const inventoryUnit = inventory.unit?.toLowerCase() || "unit"; // original inventory unit
//           const unit = stockItem.unit?.toLowerCase() || inventory.unit?.toLowerCase() || "unit"; // recipe unit
...
//           // Find all suppliers for this ingredient from supplier collection
//           const ingredientSuppliers = filteredSuppliers.filter(s => {
//             const supplierRawItem = s.rawItem || '';
//             const ingredientName = inventory.itemName || '';
            
//             if (supplierRawItem && ingredientName) {
//               const exactMatch = supplierRawItem.toLowerCase() === ingredientName.toLowerCase();
//               const containsMatch = supplierRawItem.toLowerCase().includes(ingredientName.toLowerCase()) ||
//                                     ingredientName.toLowerCase().includes(supplierRawItem.toLowerCase());
//               const wordMatch = supplierRawItem.toLowerCase().split(' ').some(word => 
//                 word.length > 2 && ingredientName.toLowerCase().includes(word)
//               ) || ingredientName.toLowerCase().split(' ').some(word => 
//                 word.length > 2 && supplierRawItem.toLowerCase().includes(word)
//               );
...
//           // Debug: Log supplier matching
//           console.log(`🔍 Finding suppliers for ingredient: ${inventory.itemName}`);
//           console.log(`📦 Found ${ingredientSuppliers.length} suppliers:`, ingredientSuppliers.map(s => ({
//             name: s.supplierName,
//             rawItem: s.rawItem,
//             id: s._id
//           })));
...
//             displayRate: `${inventoryRate}/${inventoryUnit}`, // original inventory rate for UI
//             totalCost: totalCost,
//             supplier: supplier?.supplierName || inventory.supplierName || "No Supplier Data",
//             unit: unit,
...
//             previousPurchaseDate: previousPurchase?.stock?.purchasedAt || null,
//             suppliers: ingredientSuppliers.map(s => {
//               // Find actual inventory data for this supplier and ingredient
//               const supplierInventory = filteredInventories.find(inv => 
//                 String(inv.supplierId) === String(s._id) && 
//                 inv.itemName === inventory.itemName
//               );
              
//               // console.log(`🔍 Finding inventory for supplier ${s.supplierName} and item ${inventory.itemName}:`, supplierInventory);
              
//               // Use same logic as inventory management table
//               // If supplierInventory found, use its data, otherwise use current inventory data
//               const actualQuantity =  inventoryQuantity;
//               const actualAmount =  inventoryRate;
//               const actualTotalAmount = actualAmount * actualQuantity;
              
//               // Calculate actual rate same as inventory management table
//               const actualRate = actualQuantity > 0 ? actualAmount  : 0;
              
              
//               // Return same structure as inventory management table
//               return {
//                 _id: s._id,
...
//                 // Add same fields as inventory management table
//                 category: inventory.unit || 'General',
//                 previousRate: previousRate.toFixed(2),
//                 currentRate: actualRate.toFixed(2),
//                 rateChange: rateChange,
...
//                       : `₹${filteredData.reduce((sum, item) => sum + item.totalIngredientCost, 0).toFixed(2)}`
//                     }
//                   </Typography>
...
//                               ₹{parseFloat(item.totalIngredientCost || 0).toFixed(2)}
//                             </Typography>
//                           </TableCell>
//                           <TableCell>
...
//                                           ₹{parseFloat(ingredient.totalCost || 0).toFixed(2)}
//                                         </Typography>
//                                       </TableCell>
//                                       <TableCell>
...
//                                             size="small"
//                                             onClick={() => {
//                                               setSelectedItemSuppliers(ingredient.suppliers);
//                                               setSelectedItemName(ingredient.name);
//                                               setSelectedItemUnit(ingredient.unit);
//                                               setSuppliersModalOpen(true);
//                                             }}
//                                             sx={{
//                                               fontSize: '0.75rem',
...
//                                             }}
//                                           >
//                                             View Suppliers ({ingredient.suppliers.length})
//                                           </Button>
//                                         ) : (
//                                           <Typography variant="caption" color="text.secondary" sx={{ fontStyle: 'italic' }}>
//                                             No suppliers
...
//                     <TableCell sx={{ fontWeight: 'bold', fontSize: '0.875rem' }}>Raw Item</TableCell>
...
//                         </Typography>
//                         <Typography variant="caption" color="text.secondary">
//                           ID: {supplier.supplierId ? String(supplier.supplierId).slice(-6) : 'N/A'}
//                         </Typography>
//                       </TableCell>
//                       <TableCell>
//                         <Typography variant="body2">
//                           {supplier.rawItem || selectedItemName}
//                         </Typography>
//                         <Typography variant="caption" color="text.secondary">
//                           Item: {supplier.itemName}
//                         </Typography>
//                       </TableCell>
//                       <TableCell>
//                         <Typography variant="body2">
...
//                       <TableCell>
//                         <Typography variant="body2">
//                           {supplier.unit || selectedItemUnit || 'N/A'}
//                         </Typography>
//                       </TableCell>
//                       <TableCell>
//                         <Typography variant="body2" fontWeight="medium" color="primary.main">
//                           ₹{parseFloat(supplier.amount || 0).toFixed(2)}
//                         </Typography>
//                         <Typography variant="caption" color="text.secondary">
//                           per {supplier.unit || 'unit'}
//                         </Typography>
//                       </TableCell>
//                       <TableCell>
//                         <Typography variant="body2" fontWeight="bold" color="success.main">
//                           ₹{parseFloat(supplier.total || 0).toFixed(2)}
//                         </Typography>
//                         <Typography variant="caption" color="text.secondary">
//                           (from inventory)
//                         </Typography>
...
export default function PurchaseAnalytics() {
  ...
  useEffect(() => {
    try {
      ...
      console.log("Menu items validation:", menuItemsValid);
      console.log("Inventories validation:", inventoriesValid);
      console.log("Suppliers validation:", suppliersValid);
      
      // Allow menu items transformation even without suppliers
      ...
      console.log("Filtered Suppliers for Restaurant:", filteredSuppliers);
      console.log("Suppliers available for menu items:", suppliers.length > 0 ? "Yes" : "No");
      ...
      const menuItemsWithCosts = filteredMenuItems.map(menuItem => {
        const ingredients = menuItem.stockItems?.map(stockItem => {
          ...
          const supplier = filteredSuppliers.length > 0 ? filteredSuppliers.find(
            s => String(s._id) === String(inventory.supplierId)
          ) : null;
          ...
          if (unit === "kg") {
            ...
            currentRate = inventoryRate; // per ml
          } else if (unit === "ml") {
            currentRate = inventoryRate/1000; // already per ml
          } else if (unit === "pcs") {
            currentRate = inventoryRate; // per piece
          } else {
            currentRate = inventoryRate; // fallback
      ...
      setMenuItemsWithIngredients(menuItemsWithCosts);
      console.log("Menu items with ingredients set successfully, length:", menuItemsWithCosts.length);
      setIsDataLoading(false);
    } else {
      console.log("Menu items transformation skipped - missing data:");
      console.log("Menu Items length:", menuItems.length);
      console.log("Inventories length:", inventories.length);
      console.log("Suppliers length:", suppliers.length);
    }
    } catch (error) {
      console.error("Error in menu items transformation:", error);
    }
  }, [menuItems, inventories, suppliers]);
