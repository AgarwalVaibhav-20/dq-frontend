...
const Supplier = () => {
  ...
  const [viewRawItemsModal, setViewRawItemsModal] = useState(false);
  ...
  const handleRawItemChange = (index, value) => {
    const updatedRawItems = [...formData.rawItems];
    updatedRawItems[index] = value;
    setFormData({ ...formData, rawItems: updatedRawItems });
  };

  const addRawItemField = () => {
    setFormData({ ...formData, rawItems: [...formData.rawItems, ""] });
  };

  const removeRawItemField = (index) => {
    const updatedRawItems = formData.rawItems.filter((_, i) => i !== index);
    setFormData({ ...formData, rawItems: updatedRawItems });
  };
  ...
  //     })
  //   );
  //   // await dispatch(fetchSuppliers({ restaurantId, token })); // ❌ REMOVE THIS LINE
  ...
  const renderRawItemsInput = () => (
    <div>
      {formData.rawItems.map((item, index) => (
        <CRow key={index} className="mb-2 align-items-center">
          <CCol xs={10}>
            <CFormInput
              placeholder={`Raw Item ${index + 1}`}
              value={item}
              onChange={(e) => handleRawItemChange(index, e.target.value)}
            />
          </CCol>
          <CCol xs={2} className="d-flex gap-1">
            {index === formData.rawItems.length - 1 && (
              <CButton
                color="success"
                size="sm"
                onClick={addRawItemField}
                style={{ padding: '4px 8px' }}
              >
                <CIcon icon={cilPlus} size="sm" />
              </CButton>
            )}
            {formData.rawItems.length > 1 && (
              <CButton
                color="danger"
                size="sm"
                onClick={() => removeRawItemField(index)}
                style={{ padding: '4px 8px' }}
              >
  ...
  const renderAddSupplierModal = () => (
    ...
    >
      ...
      <CModalBody>
        ...
        <CRow>
          <CCol xs={12} className="mb-3">
            <label className="mb-2 fw-bold">Raw Items</label>
            {renderRawItemsInput()}
          </CCol>
        </CRow>
      </CModalBody>
  ...
  const renderViewRawItemsModal = () => (
    <CModal
      visible={viewRawItemsModal}
      onClose={() => setViewRawItemsModal(false)}
      className="modal-responsive"
    >
      <CModalHeader>
        <CModalTitle>
          Raw Items - {selectedSupplier?.supplierName}
        </CModalTitle>
      </CModalHeader>
      <CModalBody>
        {selectedSupplier?.rawItems && selectedSupplier.rawItems.length > 0 ? (
          <div className="d-flex flex-wrap gap-2">
            {selectedSupplier.rawItems.map((item, index) => (
              <CBadge key={index} color="primary" className="p-2">
                {item}
              </CBadge>
            ))}
          </div>
        ) : (
          <p>No raw items added</p>
        )}
      </CModalBody>
      <CModalFooter>
        <CButton
          color="secondary"
          onClick={() => setViewRawItemsModal(false)}
          size="sm"
        >
          Close
        </CButton>
      ...
      renderCell: (params) => (
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
          {params.value && params.value.length > 0 ? (
            <>
              {params.value.slice(0, 2).map((item, index) => (
                <CBadge key={index} color="info" style={{ fontSize: '0.7rem' }}>
                  {item}
                </CBadge>
              ))}
              {params.value.length > 2 && (
                <CBadge color="secondary" style={{ fontSize: '0.7rem' }}>
                  +{params.value.length - 2}
                </CBadge>
              )}
            </>
          ...
          <CButton
            ...
            onClick={() => {
              ...
              setFormData({
                ...
                rawItems: params.row.rawItems?.length > 0 ? params.row.rawItems : [""],
              });
